import { isRetiredPlatformUrl, createIndependentFetch } from "@zcode/shared";
...
app.on("second-instance", (_event, _workingDirectory) => {
...
// 历史配置与插件缓存不能重新打开已退休的平台；在 Chromium 发起请求前拒绝。
app.on("session-created", (createdSession) => {
  createdSession.webRequest.onBeforeRequest((details, callback) => {
    callback({ cancel: isRetiredPlatformUrl(details.url) });
  });
});
