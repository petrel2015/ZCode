...
import { clearOAuthRoutesForWindow } from "./desktopOAuthDeepLink.js";
...
import { isRetiredPlatformUrl } from "@zcode/shared";

function isAllowedExternalOpenUrl(value: string): boolean {
  try {
    const url = new URL(value);
    return !isRetiredPlatformUrl(url) && ["http:", "https:", "file:"].includes(url.protocol);
  } catch {
    return false;
  }
}
...
export function registerRemoteIpcHandlers(options: {
  ...
  ipcMain.on(PlatformChannels.RendererReady, () => {});
